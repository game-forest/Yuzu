...
namespace Yuzu.Metadata
{
	public class Surrogate
	{
		...
		private static readonly ConcurrentDictionary<Tuple<Type, MetaOptions>, State> surrogateTypes =
		...
		private void CheckAccepts(MethodInfo m, string name, Type paramType)
		{
			...
			if (p.Length != 1) {
				throw Error("Static {0} '{1}' must have 1 parameter, but has {2}", name, m.Name, p.Length);
			}

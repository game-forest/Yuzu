...
namespace Yuzu.Clone
{
	...
	public class Cloner : AbstractCloner
	{
		...
		private readonly Dictionary<Type, Func<object, object>> clonerCache =
			new Dictionary<Type, Func<object, object>>();
		private readonly Dictionary<Type, Action<object, object>> mergerCache =
		...
		{
			return Utils.IsCopyable(t) ?? Meta.Get(t, options).IsCopyable;
		}
			...
			}
			{
				...
				if (idict != null) {
					...
					if (!IsCopyable(a[0])) {
						...
						var m = CloneUtils.GetGeneric(nameof(CloneUtils.CloneIDictionaryPrimivite), t, a[0], a[1]);
						return CloneUtils.MakeDelegate<Func<object, object>>(m);
					}
				}
			}
			...
			var meta = Meta.Get(t, Options);
			{
				...
				if (icoll != null) {
					...
					if (!IsCopyable(a[0])) {
						...
						if (meta.SerializeItemIf != null) {
							var m = CloneUtils.GetGeneric(nameof(CloneUtils.CloneCollectionPrimitiveIf), t, a[0]);
		...
		{
			{
				...
				if (idict != null) {
					...
					if (!IsCopyable(a[0])) {
						...
						var cv = GetCloner(a[1]);
						var m = CloneUtils.GetGeneric(nameof(CloneUtils.MergeIDictionaryPrimiviteKey), t, a[0], a[1]);
						var d = CloneUtils.MakeDelegate<Action<object, object, Func<object, object>>>(m);
						return (dst, src) => d(dst, src, cv);
					} else {
						var m = CloneUtils.GetGeneric(nameof(CloneUtils.MergeIDictionaryPrimivite), t, a[0], a[1]);
			...
			var meta = Meta.Get(t, Options);
			{
				...
				if (icoll != null) {
					...
					if (!IsCopyable(a[0])) {
						...
						if (meta.SerializeItemIf != null) {
							var m = CloneUtils.GetGeneric(nameof(CloneUtils.MergeCollectionPrimitiveIf), t, a[0]);
		...
		public override object DeepObject(object src) => src == null ? null : GetCloner(src.GetType())(src);

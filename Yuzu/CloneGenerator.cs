...
namespace Yuzu.Clone
{
	...
	public class ClonerGenerator : IGenerator
	{
		...
		private readonly ClonerGenerator parentGen;
		private readonly Dictionary<Type, string> generatedCloners = new Dictionary<Type, string>();
		...
		public void GenerateFooter()
		{
			...
			foreach (var r in generatedCloners.OrderBy(kv => kv.Key.Name)) {
				cw.Put(
					"clonerCache[typeof({0})] = {1};\n",
					Utils.GetTypeSpec(r.Key),
					Cloner.IsCopyable(r.Key, options)
						? "ValueCopyCloner"
						: Utils.IsStruct(r.Key)
							? r.Value + "_obj"
							: r.Value
				);
			}
		...
		private void GenerateCloneCollection(
			...
			var add = CodeWriter.GenAddToCollection(meta.Type, icoll, dstName, clonerCall);
			if (meta.SerializeItemIfMethod != null) {
				...
				cw.Put(
					"if ({0}.{1}({2}++, {3}))\n",
					srcName,
					meta.SerializeItemIfMethod.Name,
					indexName,
					itemName
				);
				cw.PutInd(add);
		...
		private void GenerateCloneItem(Meta meta, Meta.Item yi)
		{
			...
			if (yi.Type.IsArray) {
				...
				if (Cloner.IsCopyable(e, options)) {
					...
					cw.Put(
						"result.{0} = ({1})Array.CreateInstance(typeof({2}),\n",
						yi.Name,
						Utils.GetTypeSpec(yi.Type),
						Utils.GetTypeSpec(e)
					);
					cw.PutInd("new int[] {{ {0} }},\n", allDims("GetLength"));
					...
					var clonerCall = GenerateClonerInit(e, string.Format("s.{0}[{1}]", yi.Name, indexNamesStr));
		...
		private static string GetSerializeCond(Meta.Item yi)
		{
			if (yi.SerializeCond == null) {
				return null;
			}

			var result = yi.SerializeIfMethod != null
		...
		private void ActuallyGenerate(Type t, string clonerName)
		{
			...
			if (Cloner.IsCopyable(meta.Type, options)) {
				cw.Put(
					"private static {0} {1}(Cloner cl, object src) =>\n",
					Utils.GetTypeSpec(t),
					clonerName
				);
				cw.PutInd("({0})src;\n", Utils.GetTypeSpec(t));

	...
	public class JsonDeserializer : AbstractReaderDeserializer
	{
		...
		protected DateTimeOffset RequireDateTimeOffset()
		{
			...
			return DateTimeOffset.ParseExact(
				s,
				JsonOptions.DateTimeOffsetFormat,
		...
		protected static Dictionary<Type, Func<string, object>> keyParsers =
		...
		protected Array ReadArrayNDim(Type t)
		{
			...
			readRecursive = dim => {
				...
				if (lengths[dim] == 0) {
					...
					throw Error(
						"Inconsistent length of dimension {0}: expected {1}, found {2}",
						dim,
						lengths[dim],
						count
					);
		...
		private readonly Dictionary<Type, Func<object>> readerCache = new Dictionary<Type, Func<object>>();
		...
		{
			{
				...
				if (simpleReaders.TryGetValue(t, out Func<object> result)) {
					return result;
				}
			}
			if (t == typeof(decimal)) {
			...
			try {
				...
				if (JsonOptions.Unordered) {
					var storage = !Options.AllowUnknownFields || meta.GetUnknownStorage == null
						? NullYuzuUnknownStorage.Instance
						: meta.GetUnknownStorage(obj);
					storage.Clear();
		...
		private Surrogate GetSurrogate<T>(Type actualType)
		{
			...
			if (sg.FuncFrom == null) {
				throw Error(
					...
					typeof(T),
					actualType == null ? "number" : TypeSerializer.Serialize(actualType)
				);
			}
			...
			where T : class
		{
			...
			switch (ch) {
				...
				default:
					...
					// TODO: Optimize
					return (T)sg.FuncFrom(ReadValueFunc(sg.SurrogateType)());
			}
		}
			...
			switch (RequireBracketOrNull()) {
				...
				case '{':
					if (
						...
						var m = Utils.GetPrivateCovariantGenericAll(
							GetType(), nameof(ReadIntoDictionaryNG), expectedType
						);
